...
export class RealTimeTracker {
  ...
  private handleWebSocketClose(event: CloseEvent): void {
    console.log('WebSocket connection closed:', (event as any).code, (event as any).reason)
    this.onConnectionStatus?.('disconnected')

    // Attempt to reconnect if not a clean close
    if ((event as any).code !== 1000 && this.reconnectAttempts < this.maxReconnectAttempts) {
      setTimeout(() => {
  ...
  // ⬇⬇⬇ CHANGED: cross-platform timer type (Node & browser) + safe reset
